    ...
    <div class="col-lg-6 mb-4 mb-lg-0 px-lg-0 mt-lg-0">
        <div class="sticky" style="top: 85px;">
            <div class="sticky-inner">
                <a class="btn btn-lg btn-block btn-primary rounded-0 py-4 mb-3" href="{{ route('forum.create') }}">Buat Post Forum Baru</a>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="post">
                        <div class="post-header">
                            @if ($forum->user->profile && $forum->user->profile->profile_image)
                                <img src="{{ asset('images/' . $forum->user->profile->profile_image) }}" alt="Profile Image" style="flex-shrink: 0; width: 60px; height: 60px;">
                            @else
                                <img src="{{ asset('image/TRADITOUR.png') }}" alt="Gambar Profil" style="flex-shrink: 0; width: 60px; height: 60px;">
                            @endif
                            <div>
                            ...
                            <div class="modal-content" style="color: black; border-radius: 10px;">
                                ...
                                <div id="commentList{{ $forum->id }}">
                                    @if ($forum->comments->isEmpty())
                                        <p class="no-comments" style="color: black">Belum ada komentar. Silahkan berkomentar pada postingan ini.</p>
                                    @else
                                        @foreach ($forum->comments as $comment)
                                            <div class="comment-item" style="display: flex; align-items: flex-start; margin-bottom: 10px;">
                                                ...
                                                @else
                                                    <img class="comment-avatar" src="{{ asset('image/TRADITOUR.png') }}" alt="Profile Picture" style="flex-shrink: 0; width: 40px; height: 40px; margin-right: 10px;">
                                                @endif  
                                        ...
                                        <form method="POST" action="{{ route('comments.store', $forum->id) }}" class="form-group" style="flex: 1;">
                                            @csrf
                                            <label for="content" style="color: rgb(0, 0, 0); border-radius:8px; padding-bottom:15px">Tambahkan komentar:</label>
                                            <div style="position: relative;">
<textarea class="form-control" id="content" name="comment" rows="1" style="background-color: rgb(255, 255, 255); color: rgb(0, 0, 0); padding-right: 40px; height: 50px;" placeholder="Tambahkan komentar"></textarea>
<button type="submit" class="btn btn-primary" style="background-color: rgb(0, 0, 0); border-color:rgb(255, 254, 254); position: absolute; right: 0; top: 0; bottom: 0; margin: auto; width: 100px; height: 50px;">Submit</button>
                                            </div>
                                        </form>
                                    @endif
                                </div>
